...
1. Sends a GET request to the **ScrapTik API** using your TikTok `user_id`
2. Reads the existing CSV data and checks for duplicate post IDs
3. Appends **new posts** (if available) and **updates existing records**
4. Saves the cleaned, updated dataset to a local **CSV file**
5. The CSV is then linked to a **real-time Power BI dashboard**, refreshing automatically
...
# 1. Send request to ScrapTik API
url = "https://scraptik.p.rapidapi.com/user/posts"
headers = {"X-RapidAPI-Key": "your_api_key", "X-RapidAPI-Host": "scraptik.p.rapidapi.com"}
params = {"user_id": "your_user_id", "limit": "100"}
response = requests.get(url, headers=headers, params=params)
new_data = response.json()['data']

# 2. Read existing CSV
try:
    existing_df = pd.read_csv('tiktok_data.csv')
    existing_ids = existing_df['id'].astype(str).tolist()
except FileNotFoundError:
    existing_df = pd.DataFrame()
    existing_ids = []

# 3. Update existing and add new
new_records = [post for post in new_data if str(post['id']) not in existing_ids]
new_df = pd.json_normalize(new_records)
combined_df = pd.concat([existing_df, new_df], ignore_index=True)

# 4. Save final updated dataset
combined_df.to_csv('tiktok_data.csv', index=False)
